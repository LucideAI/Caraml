    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4].map((i) => (
        <Star
          key={i}
          size={10}
          className={i <= count ? 'text-amber-400 fill-amber-400' : 'text-t-ghost'}
        />
      ))}
    </div>
  );
...
function GradeBadge({ grade }: { grade?: number | null }) {
  if (grade === null || grade === undefined) {
    return (
      <span className="flex items-center gap-1 text-xs text-t-ghost">
        <Circle size={12} />
        Not attempted
      </span>
    );
...
function ExerciseGroup({
  ...
  return (
    <div className={depth > 0 ? 'ml-3 border-l border-ide-border pl-3' : ''}>
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="flex items-center gap-2 w-full py-2.5 px-3 rounded-lg hover:bg-ide-hover transition-colors group text-left"
      >
        {isOpen ? (
        ...
        <span className="text-sm font-medium text-t-secondary flex-1 truncate">
          {group.title}
        </span>
        ...
        {total > 0 && (
          <div className="w-16 h-1.5 bg-surface-2 rounded-full shrink-0 overflow-hidden">
            <div
            ...
            if (isExercise(child)) {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-3 w-full py-2.5 px-3 ml-5 rounded-lg hover:bg-ide-hover hover:border-ide-border transition-all group text-left border border-transparent"
                >
                  {/* Grade indicator */}
                  <div className="shrink-0">
                    {grade !== undefined && grade !== null && grade >= 100 ? (
                      ...
                      <Circle size={16} className="text-t-ghost group-hover:text-t-faint" />
                    )}
                  </div>

                  {/* Exercise info */}
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-t-secondary group-hover:text-t-primary truncate">
                      {child.title || child.id}
                    </div>
                    {child.short_description && (
                      <div className="text-xs text-t-faint truncate mt-0.5">
                        {child.short_description}
                      </div>
                    )}
                  </div>
                  ...
                  <ChevronRight
                    size={14}
                    className="text-t-ghost group-hover:text-t-muted shrink-0 transition-colors"
                  />
                </button>
              );
            }
      ...
      <div className="h-screen flex flex-col bg-ide-bg">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center max-w-md">
            ...
            <h2 className="text-xl font-bold text-t-primary mb-2">Connect to Learn OCaml</h2>
          ...
          {totalExercises > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
              <div className="panel p-4 text-center">
                ...
                <div className="text-lg font-bold text-t-primary">{totalExercises}</div>
                <div className="text-xs text-t-faint">Exercises</div>
              </div>
              <div className="panel p-4 text-center">
                ...
                <div className="text-xs text-t-faint">Completed</div>
              </div>
              <div className="panel p-4 text-center">
          ...
          {totalExercises > 0 && (
            <div className="mb-6">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs text-t-faint">Overall Progress</span>
          ...
          {learnOcaml.isLoadingExercises && learnOcaml.exercises.length === 0 ? (
            <div className="flex items-center justify-center py-20">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm text-t-muted">Loading exercises...</p>
              </div>
            </div>
          ) : learnOcaml.exercises.length === 0 ? (
            <div className="text-center py-20">
              <BookOpen size={48} className="mx-auto text-t-ghost mb-4" />
              <p className="text-lg text-t-muted mb-2">No exercises found</p>
                ...
                if (isExercise(item)) {
                  ...
                  return (
                    <button
                      ...
                      className="flex items-center gap-3 w-full py-3 px-4 rounded-lg hover:bg-ide-hover transition-all group text-left border border-transparent hover:border-ide-border"
                    >
                      <div className="shrink-0">
                        {grade !== undefined && grade !== null && grade >= 100 ? (
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {item.short_description && (
                          <div className="text-xs text-t-faint mt-0.5">{item.short_description}</div>
                        )}
                      </div>
                      {item.stars !== undefined && item.stars > 0 && <DifficultyStars count={item.stars} />}
                      <GradeBadge grade={grade} />
                      <ChevronRight size={14} className="text-t-ghost group-hover:text-t-muted shrink-0" />
                    </button>
                  );
                }
                return null;
